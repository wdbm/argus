...
- display SVG files through runtime rasterisation while preserving the original `.svg` file for save and move operations,
...
SVG support needs an SVG rasteriser at runtime. Argus uses `librsvg` with Python GObject bindings when available, and falls back to `CairoSVG` when installed.
...
The save dialogue is opened with `Space` or `s` and allows one to edit the filename and save to multiple directories at once. It shows recent directories as clickable buttons, with radio buttons to list them alphabetically or by most recent use; alphabetical listing is selected by default. The dialogue-local `1` to `9` and `0` shortcuts apply to the first ten directories currently shown. A `Deselect` button clears the current directory selection without altering the stored recents list. A `Move` checkbutton specifies to move the image to selected directories (as opposed to the default behaviour of copying), and Argus then advances to the next image or clears the viewer if no images remain. An "Overwrite" checkbutton overwrites files at specified directories rather than changing the filename to address conflicts. Pressing `Enter` triggers `Save`, and `Escape` closes the dialogue. `Alt+1` to `Alt+9`, and `Alt+0` for the tenth entry save to the matching indexed recent directory.
...
Argus creates its configuration directory by default at `~/.config/scattercaster` on launch, and recent save directories are stored at `~/.config/scattercaster/recent_directories.json` (a JSON array of absolute, resolved directory paths). The list is deduplicated and kept in most-recent-first order. A directory is pushed to the front of the list when one saves successfully to it, or when one selects it with `Browse...` in the save dialogue. The save dialogue shows all recorded directories as clickable buttons when available. The numbered recent-directory shortcuts use the stored order for only the top 10 entries, with `1` being the most recent directory, `2` being the next-most recent, and `10` being the tenth-most recent. Any recorded directories that are no longer valid are ignored when loading the list of recorded directories. The list of recent directories can be cleared by selecting "Clear recent directories". The per-session last selected directories state used by `Ctrl+s` is not written to drive. Only the persistent recents list is stored between runs.
